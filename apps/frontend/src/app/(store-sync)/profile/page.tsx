...
import { useUserByEmail } from "@/hooks/queries";
...
export default function ProfilePage() {
  ...
  const { data: user } = useUserByEmail(session?.username);
  ...
  const ownerName = user?.fullName ?? session?.displayName ?? "";
  const phone = user?.phone ?? "Chưa cập nhật";
  const defaultAddress = user?.address ?? "Chưa cập nhật địa chỉ";
  const currentAddress = selectedAddress || defaultAddress;
  const mapQuery = useMemo(() => encodeURIComponent(currentAddress), [currentAddress]);
              ...
              <Card className="w-full border-outline-variant">
                <CardHeader className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Chọn nhanh địa chỉ để kiểm tra vị trí hiển thị trên bản đồ.
                  </p>
                </CardHeader>
                <CardContent className="space-y-4">
                  {defaultAddress && (
                    <div className="flex flex-wrap gap-2">
                      <Button
                        variant={defaultAddress === currentAddress ? "default" : "outline"}
                        size="sm"
