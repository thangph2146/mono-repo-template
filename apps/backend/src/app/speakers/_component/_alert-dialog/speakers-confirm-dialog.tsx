...
import type { SpeakerConfirmAction } from "../types";

export interface SpeakersConfirmDialogProps {
  confirmAction: SpeakerConfirmAction | null;
...
export function SpeakersConfirmDialog({
  confirmAction,
  deleteMutation,
  restoreMutation,
  purgeMutation,
  ...
  return (
    <AdminConfirmActionDialog
      ...
      title={
        kind === "delete"
          ? "Đưa diễn giả vào thùng rác?"
          : kind === "restore"
            ? "Khôi phục diễn giả?"
            : "Xóa vĩnh viễn diễn giả?"
      }
      description={
        kind === "delete"
