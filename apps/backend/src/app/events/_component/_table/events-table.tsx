...
export function EventsTable({
  ...
  total, onRefresh, onClearFilters, onBulkDelete, isFetching,
}: EventsTableProps) {
  return (
    <AdminDataTable<EventRow>
      ...
      bulkActions={[{
        id: "bulk-event-delete", label: "Xóa tạm đã chọn", variant: "destructive",
        confirm: { title: "Đưa các sự kiện đã chọn vào thùng rác?", description: (rows) => `Bạn đã chọn ${rows.length} sự kiện.`, confirmLabel: "Xóa tạm", destructive: true },
        onAction: onBulkDelete,
      }]}
      footer={
        <div className="flex items-center justify-between">
          <p className="text-sm text-muted-foreground">{isLoading ? "Đang tải..." : `Tổng ${total} sự kiện`}</p>
        </div>
      }
    />
  );
