...
function DepartmentDetailInner() {
  ...
  const { user } = useAuth();
  const canUpdate = user ? canUserAccess(user, PERMISSION_CODES.DEPARTMENTS_UPDATE) : false;
    ...
    <PageSection max="full" className="min-w-0 space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <Button type="button" variant="outline" className="gap-1" onClick={() => router.push("/departments")}>
          ...
          <Card className="border border-border/70 shadow-sm">
            ...
            <CardContent className="space-y-5">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-foreground">{entity.description || "—"}</p>
              </div>
            </CardContent>
          </Card>
        </div>
