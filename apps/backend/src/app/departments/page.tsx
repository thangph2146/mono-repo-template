...
import { AlertCircle, RefreshCw, Building2, Plus } from "lucide-react";
...
function DepartmentsPageInner() {
  ...
  const listQuery = useDepartmentsListQuery(api, canWrite || true, listFilterParams);

  const trashQuery = useDepartmentsTrashQuery({
    api,
    trashPage,
    trashPageSize,
    debouncedTrashQ,
        ...
        <div>
          ...
          {user && !canWrite && (
            <p className="mt-2 text-sm font-medium text-amber-800 dark:text-amber-200/90">
              Chỉ xem: cần quyền <span className="font-mono">departments:manage</span> để thêm/sửa/xoá.
            </p>
          )}
        </div>
