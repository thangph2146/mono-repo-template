...
import type { AcademicYearConfirmAction } from "../types";

export interface AcademicYearsConfirmDialogProps {
  confirmAction: AcademicYearConfirmAction | null;
...
export function AcademicYearsConfirmDialog({
  confirmAction,
  deleteMutation,
  restoreMutation,
  purgeMutation,
  ...
  onConfirm,
  contentClassName,
}: AcademicYearsConfirmDialogProps) {
  ...
  return (
    <AdminConfirmActionDialog
      ...
      title={
        kind === "delete"
          ? "Đưa niên khóa vào thùng rác?"
          : kind === "restore"
            ? "Khôi phục niên khóa?"
            : "Xóa vĩnh viễn niên khóa?"
      }
      description={
        kind === "delete"
