...
import { Loader2, ArrowLeft, Pencil, Calendar, Clock, Camera as CameraIcon, Network, Hash, Monitor } from "lucide-react";
...
function DetailInner() {
  ...
  const { user } = useAuth();
  const canUpdate = user ? canUserAccess(user, PERMISSION_CODES.CAMERAS_UPDATE) : false;
  ...
  return (<PageSection max="full" className="min-w-0 space-y-6">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div className="flex items-center gap-3"><Button variant="outline" className="gap-1" onClick={() => router.push("/cameras")}><ArrowLeft className="size-4" /> Quay lại</Button><div><TypographyH1 className={ADMIN_PAGE_TITLE_PRIMARY_CLASS}>{e.name}</TypographyH1><p className={ADMIN_PAGE_SUBTITLE_CLASS}>Camera</p></div></div>
      {canUpdate && <Button variant="default" className="gap-2 rounded-lg px-5 font-semibold" onClick={() => router.push(`/cameras/${id}/edit`)}><Pencil className="size-4" /> Chỉnh sửa</Button>}
    </div>
    <div className="grid gap-6 lg:grid-cols-3 my-6">
      <div className="space-y-6 lg:col-span-2">
        <Card className="border border-border/70 shadow-sm">
          <CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-lg"><CameraIcon className="size-5 text-primary" /> Thông tin camera</CardTitle></CardHeader>
          <CardContent className="space-y-5">
            <div className="grid gap-5 sm:grid-cols-2">
              <div><p className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wide"><Network className="size-3" /> Mã camera</p><p className="mt-1 text-sm text-foreground">{e.code || "—"}</p></div>
              <div><p className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wide"><Monitor className="size-3" /> Địa chỉ IP</p><p className="mt-1 text-sm font-mono text-foreground">{e.ipAddress || "—"}</p></div>
            </div>
            <div className="grid gap-5 sm:grid-cols-2">
              <div><p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Cổng</p><p className="mt-1 text-sm">{e.port ?? "—"}</p></div>
              <div><p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Tên đăng nhập</p><p className="mt-1 text-sm">{e.username || "—"}</p></div>
            </div>
          </CardContent>
        </Card>
      </div>
...
export default function CameraDetailPage() { return <AdminPageGuard roles={["super_admin", "admin", "manager"]}><DetailInner /></AdminPageGuard>; }
