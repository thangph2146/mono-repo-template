...
export function getCameraColumns({ openDetail, openEdit, setConfirmAction }: { openDetail: (row: CameraRow) => void; openEdit: (row: CameraRow) => void; setConfirmAction: (a: CameraConfirmAction) => void; }): ColumnDef<CameraRow>[] {
  return [
    { accessorKey: "name", header: "Tên camera", enableColumnFilter: false, cell: ({ row, getValue }) => (<button type="button" className="font-medium text-left text-foreground hover:text-primary transition-colors" onClick={() => openDetail(row.original)}>{String(getValue())}</button>) },
    { accessorKey: "ipAddress", header: "IP", enableColumnFilter: false, cell: ({ getValue }) => <span className="text-sm font-mono">{String(getValue() ?? "—")}</span> },
...
export function getTrashColumns({ setConfirmAction }: { setConfirmAction: (a: CameraConfirmAction) => void; }): ColumnDef<CameraRow>[] {
  return [{ accessorKey: "name", header: "Tên", enableColumnFilter: false }, { accessorKey: "deletedAt", header: "Xóa lúc", enableColumnFilter: true, filterFn: (row, columnId, filterValue) => { if (filterValue == null || filterValue === "") return true; const rowVal = row.getValue(columnId) as string; if (!rowVal) return false; const [fromStr, toStr] = String(filterValue).split(","); const rowDate = rowVal.split("T")[0]; if (fromStr && rowDate < fromStr) return false; if (toStr && rowDate > toStr) return false; return true; }, meta: { filterVariant: "date-range" }, cell: ({ getValue }) => <span className="text-xs text-muted-foreground">{fmt(getValue() as string)}</span> }, { id: "actions", header: "Thao tác", enableSorting: false, enableColumnFilter: false, cell: ({ row }) => (<div className="flex flex-wrap gap-1"><Button type="button" variant="outline" onClick={() => setConfirmAction({ kind: "restore", row: row.original })}><ArchiveRestore className="size-3.5" /> Khôi phục</Button><Button type="button" variant="destructive" onClick={() => setConfirmAction({ kind: "purge", row: row.original })}><Trash2 className="size-3.5" /> Xóa vĩnh viễn</Button></div>) }];
}
