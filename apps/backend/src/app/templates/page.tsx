...
import { TemplatesTable, TemplatesTrashTable, TemplatesConfirmDialog, getTemplateColumns, getTrashColumns, useColumnFiltersChange, useClearListFilters, useClearTrashFilters, useHandleConfirmAction, useConfirmAction, useTemplatesListQuery, useTemplatesTrashQuery } from "./_component";
import type { TemplateRow } from "./_component";

function TemplatesPageInner() {
  const router = useRouter(), queryClient = useQueryClient(), { user } = useAuth();
  const canWrite = user ? canUserAccess(user, PERMISSION_CODES.TEMPLATES_MANAGE) || canUserAccess(user, PERMISSION_CODES.TEMPLATES_CREATE) || canUserAccess(user, PERMISSION_CODES.TEMPLATES_UPDATE) : false;
  const invalidateAll = async () => { await queryClient.invalidateQueries({ queryKey: ["templates"] }); };
  ...
  const bulM = useMutation({ mutationFn: (i: { action: string; ids: string[] }) => api.templates.bulk(i), onSuccess: invalidateAll });
  ...
  const cols = useMemo<ColumnDef<TemplateRow>[]>(() => getTemplateColumns({ openDetail: (r) => router.push(`/templates/${r.id}`), openEdit: (r) => router.push(`/templates/${r.id}/edit`), setConfirmAction }), [setConfirmAction, router]);
  const tCols = useMemo<ColumnDef<TemplateRow>[]>(() => getTrashColumns({ setConfirmAction }), [setConfirmAction]);
  return (<PageSection max="full" className="min-w-0 space-y-6">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div><TypographyH1 className={ADMIN_PAGE_TITLE_PRIMARY_CLASS}><LayoutTemplate className={ADMIN_PAGE_TITLE_ICON_CLASS} /> Mẫu hiển thị</TypographyH1><p className={ADMIN_PAGE_SUBTITLE_CLASS}>Quản lý mẫu hiển thị.</p></div>
      <div className="flex flex-wrap items-center gap-2">
        ...
        {canWrite && <Button onClick={() => router.push("/templates/new")} className="flex h-12 items-center gap-2 rounded-lg px-6 font-bold shadow-md"><Plus className="size-5" /> Thêm mẫu hiển thị</Button>}
      </div>
    </div>
