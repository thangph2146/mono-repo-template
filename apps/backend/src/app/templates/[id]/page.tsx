...
import { Loader2, ArrowLeft, Pencil, Calendar, Clock, LayoutTemplate, Hash } from "lucide-react";
    ...
    <div className="grid gap-6 lg:grid-cols-3 my-6">
      <div className="space-y-6 lg:col-span-2">
        <Card className="border border-border/70 shadow-sm">
          <CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-lg"><LayoutTemplate className="size-5 text-primary" /> Thông tin mẫu</CardTitle></CardHeader>
          <CardContent className="space-y-5">
            <div className="grid gap-5 sm:grid-cols-2">
              <div><p className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wide"><LayoutTemplate className="size-3" /> Mã mẫu</p><p className="mt-1 text-sm text-foreground">{e.code || "—"}</p></div>
              <div><p className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wide"><Hash className="size-3" /> Trạng thái</p><p className="mt-1 text-sm"><Badge variant={e.status === 1 ? "default" : "outline"}>{e.status === 1 ? "Hoạt động" : "Khóa"}</Badge></p></div>
            </div>
          </CardContent>
        </Card>
      </div>
      <div className="space-y-6">
        <Card className="border border-border/70 shadow-sm">
          <CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-lg"><LayoutTemplate className="size-5 text-primary" /> Trạng thái</CardTitle></CardHeader>
...
export default function TemplateDetailPage() { return <AdminPageGuard roles={["super_admin", "admin", "manager"]}><DetailInner /></AdminPageGuard>; }
