...
function MajorsPageInner() {
  ...
  const listQuery = useMajorsListQuery(api, canWrite || true);
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          {user && !canWrite && (
            <p className="mt-2 text-sm font-medium text-amber-800 dark:text-amber-200/90">
              Chỉ xem: cần quyền <span className="font-mono">majors:manage</span> để thêm/sửa/xoá.
            </p>
          )}
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          {canWrite && (
            <Button
              type="button"
              onClick={() => router.push("/majors/new")}
