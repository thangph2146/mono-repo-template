...
import type { TrainingSystemConfirmAction } from "../types";

export interface TrainingSystemsConfirmDialogProps {
  confirmAction: TrainingSystemConfirmAction | null;
...
export function TrainingSystemsConfirmDialog({
  confirmAction,
  deleteMutation,
  restoreMutation,
  purgeMutation,
  ...
  onConfirm,
  contentClassName,
}: TrainingSystemsConfirmDialogProps) {
  ...
  return (
    <AdminConfirmActionDialog
      ...
      title={
        kind === "delete"
          ? "Đưa hệ đào tạo vào thùng rác?"
          : kind === "restore"
            ? "Khôi phục hệ đào tạo?"
            : "Xóa vĩnh viễn hệ đào tạo?"
      }
      description={
        kind === "delete"
          ? `«${row.name}» (mã ${row.code || "N/A"}) sẽ ẩn khỏi hệ thống cho đến khi khôi phục.`
          : kind === "restore"
