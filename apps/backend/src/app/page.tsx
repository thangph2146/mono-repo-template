      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {STATUS_FILTERS.map((f) => (
          <Button
            ...
            className={`rounded-2xl p-4 h-auto text-left flex-col items-start border transition-all ${statusFilter === f.key
          ...
          >
            ...
            <p className={`text-sm font-semibold mt-1 ${statusFilter === f.key ? "text-primary-foreground" : "text-foreground"}`}>
              {f.label}
            </p>
          </Button>
        ))}
        ...
        {filtered.map((order) => {
          ...
          return (
            <div key={order.id} className="bg-background rounded-2xl border border-outline-variant shadow-sm overflow-hidden">
              ...
              {/* Row header */}
              <div
                className={`flex flex-wrap items-center gap-3 px-6 py-4 cursor-pointer select-none transition-colors ${order.status === "PENDING" ? "bg-warning/5 hover:bg-warning/10"
