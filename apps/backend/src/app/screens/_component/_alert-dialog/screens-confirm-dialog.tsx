...
export function ScreensConfirmDialog({ confirmAction, deleteMutation, restoreMutation, purgeMutation, onOpenChange, onConfirm, contentClassName }: {
  ...
  return (<AdminConfirmActionDialog open onOpenChange={onOpenChange} contentClassName={contentClassName} footerClassName="gap-2" icon={kind === "delete" ? <Archive className="size-5 shrink-0 text-destructive" /> : kind === "restore" ? <ArchiveRestore className="size-5 shrink-0 text-primary" /> : <Trash2 className="size-5 shrink-0 text-destructive" />} title={kind === "delete" ? "Xóa màn hình?" : kind === "restore" ? "Khôi phục màn hình?" : "Xóa vĩnh viễn?"} description={kind === "delete" ? `«${row.name}» vào thùng rác.` : kind === "restore" ? `Khôi phục «${row.name}».` : `Xóa vĩnh viễn «${row.name}». Không thể hoàn tác.`} confirmLabel={kind === "delete" ? "Xóa tạm" : kind === "restore" ? "Khôi phục" : "Xóa vĩnh viễn"} confirmDestructive={kind !== "restore"} confirmDisabled={deleteMutation.isPending || restoreMutation.isPending || purgeMutation.isPending} confirmLoading={kind === "delete" ? deleteMutation.isPending : kind === "restore" ? restoreMutation.isPending : purgeMutation.isPending} onConfirm={() => { void onConfirm(); }} />);
}
