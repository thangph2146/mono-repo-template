...
const colorPresets = [
  ...
  // --muted in light theme is #F5F5F5
  { label: "Muted", value: "#F5F5F5" },
  { label: "Dark Gray", value: "#4d4d4d" },
  // --muted in light theme is #F5F5F5, /50 -> alpha 0.5
  { label: "bg-muted/50", value: "#4d4d4d", alpha: 0.5 },
] as const
...
interface ColorPickerStore {
  ...
  setColor: (value: ColorValue, options?: { emitChange?: boolean }) => void
  setHsv: (value: HSVColorValue, options?: { emitChange?: boolean }) => void
...
function createColorPickerStore(
  ...
  const store: ColorPickerStore = {
    ...
    setColor: (value: ColorValue, options?: { emitChange?: boolean }) => {
      ...
      if (options?.emitChange !== false && callbacks?.onColorChange) {
    ...
    setHsv: (value: HSVColorValue, options?: { emitChange?: boolean }) => {
      if (!stateRef.current) return
...
function ColorPickerRootImpl(props: ColorPickerRootImplProps) {
  ...
  React.useEffect(() => {
    if (valueProp !== undefined) {
      ...
      if (color) {
        const hsv = rgbToHsv(color)
        // Sync external controlled value without re-emitting onValueChange
        store.setColor(color, { emitChange: false })
        store.setHsv(hsv, { emitChange: false })
      }
    }
  }, [valueProp, store])
