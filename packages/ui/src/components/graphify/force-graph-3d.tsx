  ...
  const fgData = useMemo(() => {
    const nodes = graph.nodes
      ...
      .map((n) => {
        ...
        return {
          ...
          val: isSelected ? 14 : isLinked ? 10 : Math.max(3, Math.min(8, 2 + degree * 0.5)),
          __degree: degree,
    ...
    const links = graph.links
      ...
      .map((l) => {
        ...
        return {
          ...l,
          __isRelated: relatedIds ? relatedIds.has(src) && relatedIds.has(tgt) : false,
    ...
    <div className="w-full h-full bg-zinc-950 overflow-hidden relative">
      <ForceGraph3D
        ...
        linkThreeObject={(l: unknown) => {
          const link = l as { source: { x: number; y: number; z: number }; target: { x: number; y: number; z: number }; __isRelated?: boolean; __isDirectConnection?: boolean };
          ...
          const radius = isDirect ? 0.6 : isRelated ? 0.4 : hasSelection ? 0.08 : 0.2;
        ...
        linkDirectionalParticleColor={(l: unknown) => {
          ...
          return selectedNode ? "rgba(255,255,255,0.05)" : "rgba(255,255,255,0.8)";
        }}
        warmupTicks={80}
        cooldownTicks={80}
        nodeThreeObject={(n: unknown) => {
          ...
          const mat = new THREE.MeshLambertMaterial({
            color: node.__isSelected ? 0xffffff : node.__isLinked ? 0xfbbf24 : node.color,
            transparent: true,
            opacity: node.__isSelected ? 1.0 : node.__isLinked ? 0.95 : isDimmed ? 0.2 : 0.92,
          });
          ...
          const opacity = node.__isSelected ? 1 : node.__isLinked ? 0.95 : selectedNode ? 0.3 : 0.7;
          ...
          if (node.__isLinked) {
            ...
            const ringMat = new THREE.MeshBasicMaterial({ color: 0xfbbf24, transparent: true, opacity: 0.25, side: THREE.DoubleSide });
            const ring = new THREE.Mesh(ringGeo, ringMat);
            ring.scale.setScalar(node.val / 3);
            ring.lookAt(new THREE.Vector3(0, 0, 1));
            group.add(ring);
