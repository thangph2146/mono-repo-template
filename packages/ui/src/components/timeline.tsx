    ...
    <div className={cn("relative space-y-8", className)}>
      ...
      {data.map((item, index) => (
        ...
        >
          ...
          >
            <div className="absolute top-1 left-0 flex size-6 items-center justify-center rounded-full border border-primary/30 bg-background text-primary shadow-sm md:top-2 md:right-[-12px] md:left-auto md:size-6">
              <span className="size-2 rounded-full bg-primary" />
            </div>
            {index % 2 !== 0 ? null : (
              <h3 className="text-lg font-bold text-primary md:text-right">
                {item.title}
              </h3>
            )}
          </div>
          ...
          >
            {index % 2 !== 0 ? (
              <h3 className="mb-3 text-lg font-bold text-primary md:text-left">
                {item.title}
              </h3>
            ) : null}
